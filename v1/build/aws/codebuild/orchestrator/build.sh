...
EVENT_BUS_FILE_BASE=$(echo "$EVENT_BUS_ZIP_FILE" | rev | cut -d. -f2- | rev)
...
EVENT_BUS_INCLUDE_LIST="$BASE_LIST_PATH${EVENT_BUS_FILE_BASE}_include.list"
...
EVENT_BUS_EXCLUDE_LIST="$BASE_LIST_PATH${EVENT_BUS_FILE_BASE}_exclude.list"
...
push_regular_environment () {
  ...
  create_compare_zip "$ZIP_FOLDER" "$EVENT_BUS_ZIP_FILE" "$EVENT_BUS_EXCLUDE_LIST" "$EVENT_BUS_INCLUDE_LIST"
  ...
  compare_zip_file "$EVENT_BUS_ZIP_FILE" "$EVENT_BUS_INITIAL_CODEPIPELINE" "true"
...
push_unstable_environment () {
  ...
  #Create Lambda ZIP file.
  create_compare_zip "$UNSTABLE_BRANCH" "$EVENT_BUS_ZIP_FILE" "$EVENT_BUS_EXCLUDE_LIST" "$EVENT_BUS_INCLUDE_LIST"
  create_deployment_zip "$base_path/$EVENT_BUS_ZIP_FILE"
  if [ "$(push_file "$base_path/$EVENT_BUS_ZIP_FILE" "s3://$S3_BUCKET/$S3_FOLDER/$UNSTABLE_BRANCH/$EVENT_BUS_ZIP_FILE")" = "fail" ]; then
    dev_push_flag="lambda"
  fi

  if [ "$dev_push_flag" = "success" ]; then
    start_codepipeline "$EVENT_BUS_UNSTABLE_CODEPIPELINE"
  else
...
if exists aws; then
  ...
  echo "Your system does not have \"aws\" CLI installed.  Please install this command."
  exit 1
fi
...
if exists jq; then
  ...
  echo "Your system does not have \"jq\" utility installed.  Please install this command."
  exit 1
fi
...
if exists zip; then
  ...
  echo "Your system does not have \"zip\".  Please install this command."
  exit 1
fi
...
if exists zipcmp; then
  ...
  echo "Your system does not have \"zipcmp\".  Please install this command."
  exit 1
fi
